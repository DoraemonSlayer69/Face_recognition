...
from AnchorBoxes import iou, convert_coordinates
...
def match_multi(weight_matrix, threshold):
   ...
   # print(overlaps)
    ...
    #print(anchor_indices_thresh_met)
    #print(threshold)
    tiny_face_anchors = np.nonzero((overlaps < threshold) & (overlaps >= mask_threshold))[0]
    tiny_face_anchors = sorted(tiny_face_anchors)
    #print(tiny_face_anchors)
    if len(anchor_indices_thresh_met) != 0:
        tiny_face_anchors = tiny_face_anchors[:len(anchor_indices_thresh_met)]
        anchor_indices_thresh_met = np.concatenate((anchor_indices_thresh_met,tiny_face_anchors))
    
    anchor_indices_thresh_met = anchor_indices_thresh_met.astype('int32')   
    gt_indices_thresh_met = ground_truth_indices[anchor_indices_thresh_met]
    gt_indices_thresh_met = gt_indices_thresh_met.astype('int32')
